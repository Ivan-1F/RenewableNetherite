...
public class NetheriteChestplateRenewableRecipe extends SpecialCraftingRecipe {
    public NetheriteChestplateRenewableRecipe(Identifier id) {
        super(id);
    }
    ...
    public boolean matches(CraftingInventory inv, World world) {
        ...
        if (!RenewableNetherite.check(inv, 4, Items.DIAMOND_CHESTPLATE)) return false;
        ...
        if (!RenewableNetherite.check(inv, 7, Items.OBSIDIAN)) return false;
        if (!RenewableNetherite.check(inv, 8, Items.WITHER_ROSE)) return false;
        return true;
    }
    ...
    public ItemStack craft(CraftingInventory inv) {
        ...
        ItemStack result = Items.NETHERITE_CHESTPLATE.getDefaultStack();
        if (tool.hasTag()) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return RenewableNetherite.netheriteChestplateRenewableRecipe;
    }
}
